...
const SingleAssignment = ({ assignment , handleDelete}) => {
    const { thumbnail, difficultyLevel, marks, title, _id , email} = assignment
    return (
        <div className="w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
            ...
            <div className="px-6 py-4">
                ...
                <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 className="px-2 text-sm text-primary">Mark: {marks}</h1>
                </div>
                ...
                <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 className="px-2 text-sm text-primary">Assignment Level: {difficultyLevel}</h1>
                </div>


                <div className="flex gap-3 mt-4">
                    <button onClick={()=>handleDelete(_id,email)} className="font-medium text-white text-base md:text-xl md:pb-2 md:px-4 py-1 px-1 rounded-lg bg-primary text-center">Delete</button>

                    <Link to={`/update/${_id}`}>
                        <button className="font-medium text-white text-base md:text-xl md:pb-2 md:px-4 py-1 px-1 rounded-lg bg-green-500 text-center">Update</button>
                    </Link>

                    <Link to={`/details/${_id}`}>
                        <button className="font-medium text-white text-base md:text-xl md:pb-2 md:px-4 py-1 px-1 rounded-lg bg-blue-500 text-center">View</button>
                    </Link>
                </div>
            </div>
        </div>
